...
check "go baked (pin in provision-system)" 'go version | grep -q "go1\.22\.5 "'
check "frogmouth seed (runs on py3.12)" 'frogmouth --version'
check "frogmouth outside uv data dir"  '[[ "$(readlink -f "$(command -v frogmouth)")" != "$HOME/.local/share/uv/"* ]]'
check "playwright chromium libs baked" 'ldconfig -p | grep -q libgbm.so.1 && ldconfig -p | grep -q libnss3.so && ldconfig -p | grep -q libatk-1.0.so.0'
check "seeds on PATH under su"         'sudo su codespace -c "command -v uv && command -v go"'
check "tmux runs (deps resolved)"      'tmux -V'
check "tmux commit marker matches"     '[ "$(cat /usr/local/share/aicoding/tmux-commit)" = 13c10f672c7a6bc64b2d4829ae550d8d6caf61fe ]'
