...
# before it leaves this machine's hook. The scrubbing itself (literal values
# from the secrets file, secret-shaped patterns, an entropy catch-all, and
# the fail-closed rule for an unreadable or partially parsed secrets file)
# lives in bin/redact-transcript, shared with the memory-lanes transcript
# harvester so there is exactly one redactor. "Fail closed" here always means
# "no spool file", never "the hook fails".

# Locates the shared redactor: an explicit override, then PATH, then the
# deployed symlink, then the blueprint checkout this hook was copied from.
_ml_redactor() {
  local c
  for c in "${REDACT_TRANSCRIPT:-}" "$(command -v redact-transcript 2>/dev/null)" \
           "$HOME/.local/bin/redact-transcript" \
           "$(dirname "${BASH_SOURCE[0]}")/../../../bin/redact-transcript"; do
...
_ml_tee() {
  local src="$1" sid="$2" off="$3" spool tmp redactor
  ...
  # No redactor found means no tee: an unscrubbed slice is never spooled.
  redactor="$(_ml_redactor)" || return 0

  tmp="$(mktemp "$spool/.tmp-XXXXXX")" || return 0
  chmod 600 "$tmp" 2>/dev/null
  # Fail closed: the redactor exits 3 on an unreadable or partially parsed
  # secrets file, and then nothing is spooled at all.
  if "$redactor" < "$src" > "$tmp" 2>/dev/null; then
